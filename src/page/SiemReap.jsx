...
export default function SiemReap() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get("/siemreap.json");
  ...
  return (
    <>
      <h1 className="p-5 mt-[100px] text-3xl font-bold text-center">Siem Reap Hotels</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-5 max-w-7xl mx-auto">
        {data.map((hotel) => (
          ...
          >
            {/* Image */}
            <Link to={`/DetailSR/${hotel.id}`} className="block"> {/* Added block class */}
